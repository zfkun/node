(function() {
  ...
  var transitionSlides = queryAll('.transitionSlide').forEach(function(el) {
    li_array.push( ['<li><a data-hash="', el.id, '">',
                    (query('h1', el) || query('h2', el)).textContent, '</a>',
                    '</li>'].join('')
                 );
  });
  ...
  // add by me
  SlideShow.prototype.first = function(slideId, dontPush){
    this.go(slideId, dontPush);
  };
  SlideShow.prototype.last = function(slideId, dontPush){
    this.go(slideId, dontPush);
  };
  var mySocket, mySocketOffline,
      supportCommands = {
        prev : 'prev',
        next : 'next',
        go : 'go', // slideId, dontPush
        first : 'first',
        last : 'last',
        note : 'showNotes',
        help : 'toggleHelp',
        ...
        resources : 'resources',
        theme : 'changeTheme',
        hightlight : 'toggleHightlight',
        '3d' : 'switch3D'
      };
  function fixedArgs(command, data) {
    if (command) {
        if (command === 'go' || command === 'first' || command === 'last') {
            if (command === 'go' && (!data || data.length == 0)) {
                return [];
            }
            if (!data) data = [];
            var slides = document.querySelectorAll('.slide'),
                v = command === 'go' ? parseInt(data[0]) || 0 : (command === 'first' ? 0 : slides.length - 1);
            data[0] = (slides[v] || {}).id;
        }
        return data;
    }
  ...
  function startSocketMonitor(slider) {
    mySocket = io.connect(location.protocol + '//' + location.host);

    mySocket.on('connect', function () {
        mySocketOffline = false;
        
        console.info('mySocketMonitor has connected!');

        mySocket.emit('set name', 'ppt', function(name){
            console.info('set name success: ', name);
        });
    });
    
    mySocket.on('ppt',function(data) {
        console.info('Received broadcast: ', data);
        var cmd, args;
        if (slider && data && (cmd = supportCommands[data.command]) && typeof slider[cmd] === 'function') {
            args = fixedArgs(data.command, data.args);
            console.info('run command -> ', cmd, ' , ', args);
            slider[cmd].apply(slider, args);
        }
    });

    mySocket.on('disconnect',function() {
        mySocketOffline = true;
        console.info('mySocketMonitor has disconnected!');
    });
  }
  
  startSocketMonitor(slideshow);
